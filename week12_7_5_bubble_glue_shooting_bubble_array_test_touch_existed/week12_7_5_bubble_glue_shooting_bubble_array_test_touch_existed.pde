...
void draw(){
  ...
  if(bulletFlying){
    bulletX += bulletVX;
    bulletY += bulletVY;
    if(bulletY<10 || touchingExistedBubble() ){
      bulletFlying=false;
      findNearestBubble();
    }
  }
}
boolean touchingExistedBubble(){
  for(int i=0; i<4; i++){//for(int y=10; y<=70; y+=20){
    for(int j=0; j<20; j++){//for(int x=10; x<490; x+=20){
      if( bubble[i][j]==1 && dist(10+j*20, 10+i*20, bulletX, bulletY) < 20 ){
        return true;
      }
    }
  }
...
void findNearestBubble(){
  int nearestI=0, nearestJ=0;
  for(int i=0; i<4; i++){//for(int y=10; y<=70; y+=20){
    for(int j=0; j<20; j++){//for(int x=10; x<490; x+=20){
      if( dist(10+j*20, 10+i*20, bulletX, bulletY) < 10 ){
        nearestI = i;
        nearestJ = j;
      }
    }
  }
  bubble[nearestI][nearestJ]=1;
}
